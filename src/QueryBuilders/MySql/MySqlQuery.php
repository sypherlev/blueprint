...
class MySqlQuery implements QueryInterface
{
    ...
    private function newUpdateEntry(string $column, $param)
    {
        if (!empty($this->columnwhitelist)) {
            if (!in_array($column, $this->columnwhitelist)) {
                throw (new BlueprintException('Column '.$column.' in update list not found in white list'));
            }
        }
    ...
    private function newColumnEntry(string $column, string $alias = "", string $table = "")
    {
    ...
    private function newGroupEntry(string $table, string $column)
    {
        if (!$this->validateColumnName($column)) {
            throw (new BlueprintException('Column '.$column.' in GROUP BY not found in white list'));
        }
        if ($table !== "" && !$this->validateTableName($table)) {
            throw (new BlueprintException('Table name '.$table.' in GROUP BY not found in white list'));
        }
    ...
    private function newBindEntry($param, string $type = ':wh') : string
    {
    ...
    private function checkOperand($variable, $param) : string
    {
    ...
    private function stripOperands($variable) : string
    {
